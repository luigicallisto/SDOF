...
# Avvio del download condizionale del database spettrale completo
database_totale_mat = load_all_spectra()
...
if Type_System == 'D':
    ...
    # Recuperiamo in modo sicuro i dati dal dizionario pre-caricato
    if NameSpectra in database_totale_mat:
        mat_data = database_totale_mat[NameSpectra]
        N_cyDBMean = np.asarray(mat_data['N_cyDBMean'])
        N_cyDBMstd = np.asarray(mat_data['N_cyDBMstd'])
    else:
        N_cyDBMean = np.zeros((1000, 21))
        N_cyDBMstd = np.zeros((1000, 21))
...
col8.metric("Permanent Displacement", f"{spostamento:.3f} m")
...
# Definiamo la griglia in base al tipo di sistema
if Type_System == 'D':
    # Se è 'D', creiamo due subplot affiancati (1 riga, 2 colonne)
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4.5))
else:
    # Se è 'ND', creiamo un unico grafico (1 riga, 1 colonna)
    fig, ax1 = plt.subplots(1, 1, figsize=(5, 3.5))
    ax2 = None  # Serve a evitare errori nei controlli successivi

# --- Primo Subplot (Sempre presente, sia per D che per ND) ---
...
# --- Secondo Subplot (Generato SOLO se Type_System è 'D') ---
if ax2 is not None:
...
# Infine, inviamo la figura a Streamlit centrandola (opzionale) o pulita
